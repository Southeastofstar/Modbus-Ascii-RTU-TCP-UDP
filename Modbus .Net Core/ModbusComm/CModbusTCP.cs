...
namespace ModbusComm
{
    ...
    #region "MODBUS TCP 报文格式 -- 使用二进制码进行发送和接收 - 不需要CRC"
    ...
    /// </summary>
    public sealed class CModbusTCP : IModbusComm
    {
        //, uint LocalStationNo
        ...
        public CModbusTCP(string DLLPassword, string TargetServerIPAddress, ushort TargetServerPort,
            ...
            ThreadLock.LockerType ThreadLockType = LockerType.ExchangeLock)
        {
            try
            {
                ...
                {
                    //The format of IP address for the TCP/IP Server is not correct, please correct it.\r\n
                ...
                ////该值必须大于 0。如果传递的值小于或等于零，则会引发 ArgumentOutOfRangeException。 
                //System.Net.ServicePointManager.SetTcpKeepAlive(true, 10, 100);
            ...
            {
                Enqueue("Modbus TCP 通讯类初始化时发生错误：" + ex.Message + "  " + ex.StackTrace);
                if (ShowMessageDialog == true)
                {
                    //MessageBox.Show("Modbus TCP 通讯类初始化时发生错误：\r\n" + ex.Message, "提示", //MessageBoxButtons.OK, //MessageBoxIcon.Information);
                }
            }
        }
        ...
        /// </summary>
        public const int PosIndexOfDataLengthInTCPReceivedBytes = CModbusFuncCode.PosIndexOfDataLengthInSocketReceivedBytes;//0x4;

        /// <summary>
        /// 接收到的字节数组中，从站地址的索引号[6]
        /// </summary>
        public const int PosIndexOfSlaveAddressInTCPReceivedBytes = CModbusFuncCode.PosIndexOfSlaveAddressInSocketReceivedBytes;//0x6;

        /// <summary>
        /// 接收到的字节数组中，功能码的索引号[7]
        /// </summary>
        public const int PosIndexOfFuncCodeInTCPReceivedBytes = CModbusFuncCode.PosIndexOfFuncCodeInSocketReceivedBytes;//0x7;

        /// <summary>
        /// 接收到的字节数组中，错误码的索引号[7]
        /// </summary>
        public const int PosIndexOfErrorCodeInTCPReceivedBytes = CModbusFuncCode.PosIndexOfFuncCodeInSocketReceivedBytes;//0x7;

        /// <summary>
        /// 执行读取操作时，接收到的字节数组中，接收到的数据开始的索引号[9]
        /// </summary>
        public const int PosIndexOfDataInTCPReceivedBytes = CModbusFuncCode.PosIndexOfDataInSocketReceivedBytes;//0x9;
        ...
        public bool ReadCoilBit(byte DeviceAddress, ushort BeginAddress, ref bool Value)//ushort ReadDataLength 读取数据长度,   , out string ReadBackData)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (byReadData[PosIndexOfSlaveAddressInTCPReceivedBytes] == byDataToBeSent[PosIndexOfSlaveAddressInTCPReceivedBytes]
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        if (byReadData[PosIndexOfDataInTCPReceivedBytes] == 1)
                        {
                            Value = true;
                        }
                        else
            ...
            catch (Exception ex)
            {
                ...
                if (ShowMessageDialog == true)
                {
                    //MessageBox.Show("Modbus TCP通讯时发生错误：\r\n" + ex.Message, "提示", //MessageBoxButtons.OK, //MessageBoxIcon.Information);
                }
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ByteToBitArray(CConverter.CopyBytes(byReadData, (uint)ReadDataLength, (uint)PosIndexOfDataInTCPReceivedBytes));
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (byReadData[PosIndexOfSlaveAddressInTCPReceivedBytes] == byDataToBeSent[PosIndexOfSlaveAddressInTCPReceivedBytes]
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        Value = byReadData[PosIndexOfDataInTCPReceivedBytes];
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.CopyBytes(byReadData, (uint)ReadDataLength, (uint)PosIndexOfDataInTCPReceivedBytes);
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (byReadData[PosIndexOfSlaveAddressInTCPReceivedBytes] == byDataToBeSent[PosIndexOfSlaveAddressInTCPReceivedBytes]
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        Value = (sbyte)byReadData[PosIndexOfDataInTCPReceivedBytes];
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        for (int i = 0; i < ReadDataLength; i++)
                        {
                            byResultData[i] = (sbyte)byReadData[PosIndexOfDataInTCPReceivedBytes + i];
                        }
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt16(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Short), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt16Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Short), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt16(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.UShort), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt16Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.UShort), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt32(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Int), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt32Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Int), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt32(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.UInt), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt32Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.UInt), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToFloat(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Float), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToFloatArray(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Float), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToDouble(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Double), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToDoubleArray(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Double), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt64(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Long), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt64Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Long), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt64(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.ULong), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt64Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.ULong), (uint)PosIndexOfDataInTCPReceivedBytes), ReadCoilBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Short), (uint)PosIndexOfDataInTCPReceivedBytes);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt16(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Short), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputIOBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt16Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Short), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputIOBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt16(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.UShort), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputIOBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt16Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.UShort), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputIOBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt32(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Int), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputIOBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt32Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Int), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputIOBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt32(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.UInt), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputIOBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt32Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.UInt), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputIOBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt64(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Long), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputIOBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt64Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Long), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputIOBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt64(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.ULong), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputIOBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt64Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.ULong), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputIOBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt16(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Short), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt16Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Short), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt16(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.UShort), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt16Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.UShort), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt32(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Int), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt32Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Int), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt32(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.UInt), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt32Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.UInt), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToFloat(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Float), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToFloatArray(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Float), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToDouble(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Double), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToDoubleArray(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Double), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt64(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Long), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt64Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Long), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt64(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.ULong), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt64Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.ULong), (uint)PosIndexOfDataInTCPReceivedBytes), ReadInputRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt16(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Short), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt16Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Short), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt16(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.UShort), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt16Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.UShort), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt32(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Int), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt32Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Int), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt32(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.UInt), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt32Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.UInt), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToFloat(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Float), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToFloatArray(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Float), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToDouble(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Double), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToDoubleArray(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Double), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt64(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.Long), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToInt64Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.Long), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt64(CConverter.CopyBytes(byReadData, (uint)(1 * (byte)ByteCount.ULong), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
            ...
            try
            {
                ...
                else
                {
                        ...
                        && byReadData[PosIndexOfFuncCodeInTCPReceivedBytes] == byDataToBeSent[PosIndexOfFuncCodeInTCPReceivedBytes])
                    {
                        ...
                        Value = CConverter.ToUInt64Array(CConverter.CopyBytes(byReadData, (uint)(ReadDataLength * (byte)ByteCount.ULong), (uint)PosIndexOfDataInTCPReceivedBytes), ReadKeepRegisterBytesFormat, 0);
        ...
        public bool WriteCoilBit(byte DeviceAddress, ushort BeginAddress, bool[] SetValue)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (ShowMessageDialog == true)
                {
                    //MessageBox.Show("Modbus TCP 通讯时发生错误：\r\n" + ex.Message, "提示", //MessageBoxButtons.OK, //MessageBoxIcon.Information);
                }
            ...
            while (bIsDisposing == false)
            {
                try
                {
                    ...
                    if (null == Client)
                    {
                        ...
                        //Client = new Socket(AddressFamily.InterNetwork, SocketType.Seqpacket, ProtocolType.Tcp);//在这个地址家族中不存在对指定的插槽类型的支持。
                        //Client = new Socket(AddressFamily.InterNetwork, SocketType.Raw, ProtocolType.Tcp);//)在 getsockopt 或 setsockopt 调用中指定的一个未知的、无效的或不受支持的选项或层次。
                        Client = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
                        Client.SendTimeout = SendTimeout;
                        Client.ReceiveTimeout = ReceiveTimeout;
        ...
        private CReadbackData UnpackReceivedRTUMsg(byte[] ReceivedMsg)
        {
            ...
            try
            {
                ...
                gotData.SlaveAddress = ReceivedMsg[PosIndexOfSlaveAddressInTCPReceivedBytes];//从站地址

                gotData.FuncCode = ReceivedMsg[PosIndexOfFuncCodeInTCPReceivedBytes];//功能码
                gotData.FuncDescription = CModbusFuncCode.FuncInfo((ModbusFuncCode)gotData.FuncCode);//功能码的描述信息

                gotData.ErrorCode = ReceivedMsg[PosIndexOfErrorCodeInTCPReceivedBytes];//错误码
                gotData.ErrorMsg = AnalysisErrorCode(ReceivedMsg);//错误码的描述信息

                //数据字符串/字节数组
                int iLength = ReceivedMsg.Length;
                int iCopyDataLength = iLength - PosIndexOfDataInTCPReceivedBytes;
                if (iCopyDataLength > 0)
                {
                    gotData.DataBytes = new byte[iCopyDataLength];
                    Array.Copy(ReceivedMsg, PosIndexOfDataInTCPReceivedBytes, gotData.DataBytes, 0, iCopyDataLength);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            CReadbackData Msg = CModbusErrorCode.AnalysisErrorCode(MsgWithErrorCode, ModbusCommType.TCP);
            if (null == Msg)
            {
                return "";
            }
        ...
        ///// <summary>
        ///// 创建写多个线圈命令的字节数组，可以直接发送这个字节数组到TCP端口
        ///// </summary>
        ///// <param name="DeviceAddress">从站地址</param>
        ...
        ///// <summary>
        ///// 将16进制字符串(用字符进行分隔)转换为TCP码字符串
        ///// </summary>
        ///// <param name="HexStringSplitByChar">16进制字符串(用字符进行分隔)</param>
        ///// <param name="SplitChar">分割字符</param>
        ///// <returns></returns>
        //public static string HexStringSplitByCharToTCPString(string HexStringSplitByChar, char SplitChar = ' ')
        //{
        //    string sResult = "";
